...
const Grades = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {isLoading && <p className="text-muted-foreground">Loading grades...</p>}
      {isError && (
        <Card>
              ...
              {[
                {
                  label: "A (90-100%)",
                  percentage: 75,
                  count: grades.filter((g) => (g.percentage || 0) >= 90).length,
                },
                {
                  label: "B (80-89%)",
                  percentage: 60,
                  count: grades.filter(
                    (g) =>
                      (g.percentage || 0) >= 80 && (g.percentage || 0) < 90,
                  ).length,
                },
                {
                  label: "C (70-79%)",
                  ...
                  count: grades.filter(
                    (g) =>
                      (g.percentage || 0) >= 70 && (g.percentage || 0) < 80,
                  ).length,
                },
                {
                  label: "D (60-69%)",
                  ...
                  count: grades.filter(
                    (g) =>
                      (g.percentage || 0) >= 60 && (g.percentage || 0) < 70,
                  ).length,
                },
              ].map((item) => (
